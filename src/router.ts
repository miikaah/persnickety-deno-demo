...
export default async function getRouteHandler(req: ServerRequest): Promise<RequestHandler | undefined> {
  ...
  const { routeKey, handlerWithParams } = getHandlerWithParams(url, method, urlParts);
